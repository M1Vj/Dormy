...
import { CommitteeWidget } from "@/components/dashboard/committee-widget";
import { OccupantStanding } from "@/components/dashboard/occupant-standing";
import { Badge } from "@/components/ui/badge";
import { Activity, Star, Sparkles, UserCheck } from "lucide-react";
...
export default async function HomePage() {
  ...
  const [fineRules, events, cleaningSnapshot, balance, entries, stats, committeeResult] = await Promise.all([
    getFineRules(dormId),
  ...
  const committeeData = "data" in committeeResult ? committeeResult.data : null;
        ...
        {committeeData && committeeData.length > 0 && (
          <div className="space-y-3">
            ...
            <div className="grid gap-4">
              {committeeData.map((committee: any) => (
                <CommitteeWidget 
                  key={committee.id}
                  committee={committee}
                  role={role}
