...
export default async function TemplateTemplatePage({ params }: Props) {
  ...
  const myRole = roles.includes("admin")
    ? "admin"
    : roles.includes("student_assistant")
      ? "student_assistant"
      : roles.includes("adviser") || roles.includes("assistant_adviser")
        ? "adviser"
        : roles[0] || "occupant";
    ...
    <div className="space-y-6 p-8">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" asChild>
          <Link href={`/${myRole}/evaluation/${cycle_id}`}>
